...
interface AuthContextType {
  ...
  setDailyCaloriesGoal: (goal: number) => void;
  loadDailyCaloriesGoal: () => Promise<void>;
...
export const AuthProvider: React.FC<AuthProviderProps> = ({ children }) => {
  ...
  const login = async (email: string, password: string): Promise<boolean> => {
    try {
      ...
      // Load daily calories goal after successful login
      await loadDailyCaloriesGoal();
  ...
  const loadDailyCaloriesGoal = async (): Promise<void> => {
    try {
      ...
      if (userData && userData.DailyCalories) {
        setDailyCaloriesGoal(userData.DailyCalories);
        console.log('Daily calories goal loaded:', userData.DailyCalories);
      }
    } catch (error) {
      console.error('Error loading daily calories goal:', error);
